...
class RockPaperScissorsLizardSpock:
    ...
    def get_player_choice_from_input(self, round_number):
        ...
        while not valid_choice:
            player_choice = input(f"It's round {round_number}. Enter your ({'/'.join(self.choices)}) choice : ").lower()
            if player_choice in self.choices:
                valid_choice = True
            else:
                print(f"{constants.RED_COLOR}Invalid choice. Please try again.{constants.RESET_COLOR}")
    ...
    def get_valid_player_input(self, text_player_can_see, min_number, max_number):
        ...
        while not valid_input:
            player_input = input(text_player_can_see + f" ({min_number}-{max_number})\n  :")
    ...
    def play_game(self):
        self.print_tools.print_beginning_of_game(self.choices, self.computer_names)
        ...
        for computer_game_index in range(1, number_of_opponents + 1):
            ...
            for computer_opponent_index in range(1 + computer_game_index, number_of_opponents + 1):
                ...
                while True:
                    ...
                    result = self.determine_winner(computer_main_player_choice, cpu_played_against_choice)
