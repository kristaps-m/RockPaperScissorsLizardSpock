...
class ScoreTools:
    ...
    def create_scores_list(self, num_players):
        for i in range(num_players + 1):
            ...
            for j in range(num_players + 1):
                if i != j:
                    self.scores[i].append(scores.Scores(self.players[i], 0, self.players[j], 0))
                else:
                    self.scores[i].append(None)

    def generate_player_names_and_games_won_dictionary(self, number_of_opponents):
        self.player_names_and_games_won = {k: 0 for k in self.players[:number_of_opponents + 1]}

    def count_games_won_for_each_player(self, number_of_opponents):
        self.generate_player_names_and_games_won_dictionary(number_of_opponents)
    ...
    def sort_and_print_player_names_and_games_won(self):
        ...
        self.print_tools.print_player_name_and_victories(sorted_dict, max_name_length)

    def have_you_won_the_game_with_most_wins(self):
        max_win_score = max(wins for wins in self.player_names_and_games_won.values())
    ...
    def update_scores(self, player1, score1, player2, score2):
        ...
        self.scores[index1][index2] = scores.Scores(player1, score1, player2, score2)
        self.scores[index2][index1] = scores.Scores(player2, score2, player1, score1)
